...
Send `session.update` immediately: the first update is acknowledged with `session.created`. Use mono PCM16 little-endian at 24 kHz in both directions, base64 JSON. Configure `audio.input.format` and `audio.output.format` as `{"type":"audio/pcm","rate":24000}`; output modalities `["audio"]`; output voice `default`; input transcription `{"model":"higgs-stt-3.1","language":"en"}`; semantic VAD. The English hint was added after the founder saw Chinese under YOU while speaking English. It is an optional ISO-639-1 hint documented by the provider, not a guarantee of mixed-language fidelity. Convert hardware microphone samples to this format.
