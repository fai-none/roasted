...
| `server/src/scheduler.js`, `schedule-service.js`, `scheduling.js`; client callback actions | None from old product | **Discard old callback scheduling.** The founder selected in-app incoming calls for MVP. Scheduled/background delivery is deferred; do not port APNs/PushKit/CallKit or the old callback scheduler for this scope. |
