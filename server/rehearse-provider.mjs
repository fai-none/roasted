...
async function connect(memory = []) {
  ...
  send({ type: 'session.update', session: {
    ...
    audio: { input: { format: { type: 'audio/pcm', rate: 24000 }, transcription: { model: 'higgs-stt-3.1' }, turn_detection: { type: 'semantic_vad' } }, output: { format: { type: 'audio/pcm', rate: 24000 }, voice: 'default' } },
